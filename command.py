...
def check_command(command):
    ...
    # if 0<=x<=5 and 0<=y<=5 and position in (N,S,E,W) then proceed.
    check_place = sub_cmd_list[0].split(",")
    ...
    check_place[2].lower() in config.options('Position'):
        # Concat PLACE to start of command set  
        sub_cmd_list[0]=start_cmd+' '+sub_cmd_list[0]
        # Rest of the commands must be MOVE or LEFT or RIGHT 
